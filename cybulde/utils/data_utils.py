...
def commit_to_dvc(dvc_raw_data_folder: str, dvc_remote_name: str) -> None:
    ...
    run_shell_command(f"git commit -nm 'Updated version of the data from {current_version} to {next_version}'")
...
def make_new_data_version(dvc_raw_data_folder: str, dvc_remote_name: str) -> None:
    try:
        ...
        DATA_UTILS_LOGGER.info(f"Status {status}")
        if status.rstrip() == "Data and pipelines are up to date.":
            DATA_UTILS_LOGGER.info("Data and pipelines are UP TO DATE")
            return
        else:
            DATA_UTILS_LOGGER.info("DChange in DVC State.. New change needs to be commited to DVC")
            commit_to_dvc(dvc_raw_data_folder, dvc_remote_name)
            return
    except CalledProcessError:
        DATA_UTILS_LOGGER.info("Exception occured")
        commit_to_dvc(dvc_raw_data_folder, dvc_remote_name)
